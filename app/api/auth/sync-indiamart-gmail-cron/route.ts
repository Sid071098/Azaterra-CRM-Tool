...
import { syncIndiaMartLeadsFromGmailAccount } from "@/lib/indiamartGmailSync";
...
export async function GET(req: NextRequest) {
  ...
  for (const account of accounts) {
    try {
      results.push({
        email: account.email,
        ...(await syncIndiaMartLeadsFromGmailAccount(account)),
      });
    } catch (err) {
      results.push({
        email: account.email,
        error: err instanceof Error ? err.message : "IndiaMART Gmail sync failed",
      });
    }
  }
